...
$.fn.easyTable = function (options) {
   ...
   this.message = {
      all: 'Mark all records.',
      clear: 'Uncheck all registered.',
      search: 'Search by'
   };
   ...
   this.sortable = function () {
      ...
      this.makeSortable = function (table) {
         ...
         while (--i >= 0) {
            (function (i) {
               var dir = 1;
               $(th[i]).append(' <i data-order="up"></i>'); 
               $(th[i]).append(' <i data-order="down"></i>');
               $(th[i]).click(function () {
                  trIndex = $(th[i]).index();
   ...
   this.filter = function () {
      ...
      if (trIndex === 'all') {
         $("#search").attr('placeholder', $("#search").attr('placeholder') + ' everyone');
      } else {
      }
